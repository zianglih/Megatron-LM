...
def _install_route(
    ...
    def route(_self, hidden_states, padding_mask=None, input_ids=None):
        ...
        background = torch.zeros(
            (tokens, 1), device=route_logits.device, dtype=route_logits.dtype
        )
        selected_probs = torch.softmax(
            torch.cat((route_logits, background), dim=-1), dim=-1
        )[:, :-1]
        probs = torch.zeros(
            (tokens, NUM_EXPERTS),
...
def _build_layer(dispatcher: str, quantization: str, world_size: int) -> MoELayer:
    ...
    return MoELayer(
        config,
        MoESubmodules(experts=ModuleSpec(module=GroupedMLP)),
        layer_number=1,
    ).cuda()
